...
# # # # Simplified system prompt to avoid API issues
# # # SYSTEM_PROMPT = """
...
# # # @app.post("/")
# # # async def ultravoice_callback(request: Request):
# # #     try:
# # #         data = await request.json()
# # #         logger.info(f"Ultravox callback: {data}")

# # #         # If the model asks for a transfer, just proxy to /transfer
# # #         if data.get("toolName") == "transfer_to_human":
# # #             return await transfer_to_human()

# # #         # If it’s another tool like get_date_info
# # #         if data.get("toolName") == "get_date_info":
# # #             return await get_date_info(DateRequest(**data.get("parameters", {})))

# # #         return {"status": "ok"}

# # #     except Exception as e:
# # #         logger.error(f"Error in Ultravox callback: {e}")
# # #         return {"error": str(e)}
...
# # #         # Poll transcripts in the background
...
# Simplified system prompt
SYSTEM_PROMPT = """
You are Qazi from Wise Choice Realty, a friendly voice assistant for real estate referrals.
...
@app.post("/enqueue-transfer")
async def enqueue_transfer():
    """
    Endpoint to enqueue the call for transfer to a human
    """
    logger.info("Enqueuing call for transfer to human representative")
    # TwiML to enqueue the call
    twiml = """
    <Response>
        <Enqueue workflowSid="WW0123456789abcdef0123456789abcdef">Wait</Enqueue>
    </Response>
    """
...
async def poll_call_transcripts(call_id: str, call_sid: str):
    ...
    try:
        ...
        while True:
            # Check if 20 seconds have passed and transfer hasn't been initiated yet
            if (datetime.now() - start_time).total_seconds() >= 20 and not transfer_initiated:
                logger.info("20 seconds have passed, initiating transfer")
                transfer_initiated = True
                
                # Instead of trying to redirect the call, we'll use a different approach
                # We'll make an outbound call to the agent and conference them in
                if twilio_client and TransferNumber:
                    try:
                        # Make an outbound call to the agent
                        call = twilio_client.calls.create(
                            to=TransferNumber,
                            from_=os.getenv("TWILIO_PHONE_NUMBER"),  # Your Twilio phone number
                            url=f"{BASE_URL}/connect-agent?originalCallSid={call_sid}",
                            method="GET"
                        )
                        logger.info(f"Outbound call to agent initiated: {call.sid}")
                    except Exception as e:
                        logger.error(f"Failed to initiate outbound call to agent: {e}")
                
                # Break out of the polling loop since we've initiated transfer
                break
...
@app.get("/connect-agent")
async def connect_agent(originalCallSid: str):
    """
    Endpoint to connect the agent to the original call
    """
    logger.info(f"Connecting agent to original call: {originalCallSid}")
    
    # TwiML to dial the original call and conference them together
    twiml = f"""
    <Response>
        <Dial>
            <Conference>{originalCallSid}</Conference>
        </Dial>
    </Response>
    """
