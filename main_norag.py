...
# from norag_prompt import SYSTEM_PROMPT # Main prompt imported
from prompt_real_statte_agent import SYSTEM_PROMPT # Main prompt imported
import hashlib
...
def save_to_txt(name, email, available_time, zip_code):
    with open("customers.txt", "a") as f:
        f.write(f"{name},{email},{available_time},{zip_code}\n")
...
async def handle_incoming_call(request: Request, background_tasks: BackgroundTasks):
    try:
        payload = {
            ...
            "experimentalSettings": {"backgroundNoiseFilter": True} # New setting implemented to overcome background noise. Currently experimental as stated by Ultravox's team
        }
